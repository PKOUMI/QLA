...
check('there is an account button', await visible('#btn-account'));
check('the details start tucked away', await page.evaluate(`document.querySelector('#account-panel').hidden === true`));
await page.evaluate(`document.querySelector('#btn-account').click()`);
await sleep(200);
const account = await text('#account-panel');
check('the menu shows who is signed in', account.includes('alice@northgate.sch.uk'), account.slice(0, 60));
check('and their school', account.includes('Northgate High'));
check('and what their role lets them do', /Owner/.test(account) && /manage staff/i.test(account));
await page.evaluate(`document.body.click()`);
await sleep(200);
check('clicking away closes it', await page.evaluate(`document.querySelector('#account-panel').hidden === true`));

console.log('\n9. Coming back tomorrow');
await page.goto(APP);
await sleep(1200);
check('no second sign-in is demanded', !(await page.evaluate(`!!document.querySelector('#auth-gate')`)));
check('the account button is still there', await visible('#btn-account'));

console.log('\n10. Signing out');
await page.evaluate(`document.querySelector('#btn-account').click()`);
await sleep(200);
await page.evaluate(`[...document.querySelectorAll('#account-panel button')].find(b => /Sign out/.test(b.textContent)).click()`);
await sleep(1500);
